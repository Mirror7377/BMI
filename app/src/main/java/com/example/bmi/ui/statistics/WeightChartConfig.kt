...
class WeightChartConfig : ChartConfig<DayWeightData> {
    ...
    override fun getDotColor(value: Float): Color = Color.White
    ...
    override fun DrawScope.drawSelectedDot(point: Offset, value: Float) {
        drawCircle(Color.White, radius = selectedDotRadiusDp.dp.toPx(), center = point)
    }

    // 对应原 WeightChartView.drawDots() 普通状态
    override fun DrawScope.drawNormalDot(point: Offset) {
        drawCircle(Color.White, radius = normalDotRadiusDp.dp.toPx(), center = point)
    }
